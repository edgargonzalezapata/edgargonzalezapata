...
namespace SIMPLE_API.Cesion
{
    ...
    public class AEC
    {
        ...
        public string Version { get { return ChileSystems.DTE.Engine.Config.Resources.versionAEC; } set { } }
        ...
        public string Firmar(string nombreCertificado, out string message, string outputDirectory = "out\\temp\\", string password = "")
        {
            ...
            try
            {
                ...
                AppendXML(filePath);
                var content = SIMPLE_API.Security.Firma.Firma.FirmarDocumentoPath(filePath, this.DocumentoAEC.ID, nombreCertificado, password);
            }
            catch (Exception ex)
            {
                filePath = ex.Message;
