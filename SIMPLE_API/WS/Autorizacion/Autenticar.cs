...
namespace ChileSystems.DTE.WS.Autorizacion
{
        ...
        public static string GetToken(string nombreCertificado, AmbienteEnum ambiente, out string message, string fullTokenPath, string password = "")
        {
            ...
            for (int i = 0; i < TRY_NUM; i++)
            {
                try
                {
                    ...
                    {
                        SIMPLE_API.Produccion.Seed.CrSeedService seed = new SIMPLE_API.Produccion.Seed.CrSeedService();
                        s = seed.getSeed();
                    }
                    else
                    {
                        SIMPLE_API.Certificacion.Seed.CrSeedService seed = new SIMPLE_API.Certificacion.Seed.CrSeedService();
                        s = seed.getSeed();
                    }
                    ...
                    if (!String.IsNullOrEmpty(s))
                    {
                        ...
                        {
                            string signedSeedXml = SIMPLE_API.Security.Firma.Firma.firmarDocumentoSemilla(formattedSeed, nombreCertificado, password);
                            ...
                            if (ambiente == AmbienteEnum.Produccion)
                            {
                                ...
                                SIMPLE_API.Produccion.Token.GetTokenFromSeedService token = new SIMPLE_API.Produccion.Token.GetTokenFromSeedService();
                                t = token.getToken(signedSeedXml);
                            }
                            else
                            {
                                SIMPLE_API.Certificacion.Token.GetTokenFromSeedService token = new SIMPLE_API.Certificacion.Token.GetTokenFromSeedService();
                                t = token.getToken(signedSeedXml);
                            }                             
